...
func (rs *Store) Query(req *types.QueryRequest) (*types.QueryResponse, error) {
	path := req.Path
	...
	if store == nil {
		return &types.QueryResponse{}, errorsmod.Wrapf(types.ErrUnknownRequest, "no such store: %s", storeName)
	}

	queryable, ok := store.(types.Queryable)
	if !ok {
		return &types.QueryResponse{}, errorsmod.Wrapf(types.ErrUnknownRequest, "store %s (type %T) doesn't support queries", storeName, store)
	}
	...
	if res.ProofOps == nil || len(res.ProofOps.Ops) == 0 {
		return &types.QueryResponse{}, errorsmod.Wrap(types.ErrInvalidRequest, "proof is unexpectedly empty; ensure height has not been pruned")
	}
