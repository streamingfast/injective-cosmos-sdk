...
func LoadStore(db dbm.DB, id types.CommitID, lazyLoading bool, cacheSize int, commitSync bool) (types.CommitKVStore, error) {
	return LoadStoreWithInitialVersion(db, id, lazyLoading, 0, cacheSize, commitSync)
}
...
func LoadStoreWithInitialVersion(db dbm.DB, id types.CommitID, lazyLoading bool, initialVersion uint64, cacheSize int, commitSync bool) (types.CommitKVStore, error) {
	tree, err := iavl.NewMutableTreeWithOpts(db, cacheSize, &iavl.Options{InitialVersion: initialVersion, Sync: commitSync})
	if err != nil {
		return nil, err
	}
