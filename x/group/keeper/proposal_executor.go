...
func (s Keeper) doExecuteMsgs(ctx sdk.Context, router baseapp.MessageRouter, proposal group.Proposal, groupPolicyAcc sdk.AccAddress, decisionPolicy group.DecisionPolicy) (meterResult []sdk.Result, err error) {
	defer s.Meter(ctx).FuncTiming(&ctx, "doExecuteMsgs")(&err)
